...
from my import concept_tree_init
from utils import load_obj, tensor2val
...
def get_mapper(root):
    ...
    e_dict = dict(zip(sorted(e), range(len(e))))
    c_dict = dict(zip(sorted(c), range(len(c))))
    r_dict = dict(zip(sorted(r), range(len(r))))
...
class TAR(torch.nn.Module):
    def __init__(self, emb_dim, e_dict, c_dict, r_dict):
        ...
        # concept_tree_init(c_dict, self.c_embedding.weight.data)
    ...
    def forward(self, x):
        ...
        # return qe_losses, cc_loss, ec_loss
        # return qe_losses, qc_losses, ec_loss
...
def dataloader(cfg, e_dict, c_dict, r_dict, query_type):
    if query_type in ["1p", "2p", "3p", "2i", "3i"]:
        code = f'''
train_e_{query_type}, train_c_{query_type}, train_filter_e_{query_type}, train_filter_c_{query_type}, \
test_e_{query_type}, test_c_{query_type}, test_filter_e_{query_type}, test_filter_c_{query_type}, \
valid_dataloader_{query_type}_e, valid_dataloader_{query_type}_c, test_dataloader_{query_type}_e, test_dataloader_{query_type}_c = \
load_train_and_test_data(cfg.root + cfg.dataset + '/', cfg.num_workers, e_dict, c_dict, r_dict, query_type="{query_type}")
        '''
    else:
        code = f'''
test_e_{query_type}, test_c_{query_type}, test_filter_e_{query_type}, test_filter_c_{query_type}, \
valid_dataloader_{query_type}_e, valid_dataloader_{query_type}_c, test_dataloader_{query_type}_e, \
test_dataloader_{query_type}_c = load_test_data(cfg.root + cfg.dataset + '/', cfg.num_workers, \
e_dict, c_dict, r_dict, query_type="{query_type}")
            '''
    exec(code)
    return locals().get(f'test_dataloader_{query_type}_e', None), locals().get(f'test_filter_e_{query_type}', None)
...
if __name__ == '__main__':
    ...
    for epoch in range(cfg.max_epochs):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            # 无qc
            # qe_losses, cc_loss, ec_loss = model(batch)
            # qe_loss = sum(qe_losses) / len(qe_losses)
            # loss = (qe_loss + cc_loss + ec_loss) / 3

            # 无cc
            # qe_losses, qc_losses, ec_loss = model(batch)
            # qe_loss = sum(qe_losses) / len(qe_losses)
            # qc_loss = sum(qc_losses) / len(qc_losses)
            # loss = (qe_loss + qc_loss + ec_loss) / 3

            # 多任务
            # loss_list = [qe_loss, qc_loss, cc_loss, ec_loss]
            # loss_list = torch.Tensor([qe_loss, qc_loss, cc_loss, ec_loss])
            # loss_list.to(device)
            # final_loss = []
            # for i in range(4):
            #     final_loss.append(
            #         loss_list[i] / (2 * model.uncertainty_weight[i] ** 2) + torch.log(model.uncertainty_weight[i]))
            # loss = sum(final_loss)
            # loss = sum(loss_list) / 4
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            avg_loss.append(loss.item())
            if batch_idx == len(train_dataloader) - 1:
                print(f'qe: {round(tensor2val(qe_loss), 4)}, qc: {round(tensor2val(qc_loss), 4)},'
                      f'cc: {round(tensor2val(cc_loss), 4)}, ec: {round(tensor2val(ec_loss), 4)}')
